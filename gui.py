...
def get_total_seconds():
    try:
        ...
        second = int(sec1.get()) * 10 + int(sec2.get())
...
def countdown_timer(seconds):
    ...
    while running_flag[0] and time.monotonic() < end_time:
...
def start_activity():
    ...
    if total_seconds == 0:
        messagebox.showwarning("Input Error", "Please input a valid time greater than 00:00:00.")
        return
    ...
    auto.mouse.use_modifiers = use_modifiers_var.get()
    name = tracker_app_var.get().strip()
    auto.mouse.tracker_app_name = name if name else None
    running_flag[0] = True
    mins = total_seconds // 60
    log_message(f"[System] Starting AutoActivity for {mins} minute(s) with: {selected_apps}")
    log_message("[System] Starting in 5 seconds...")

    def delayed_start():
        time.sleep(5)
        auto.start(selected_apps)
    ...
    threading.Thread(target=delayed_timer, daemon=True).start()
...
tk.Label(tab_main, text="Set Timer (HH:MM:SS)", font=("Arial", 12)).pack(pady=(10, 0))
...
sec1 = tk.Spinbox(time_frame, from_=0, to=5, width=2, font=("Arial", 14), justify='center')
sec2 = tk.Spinbox(time_frame, from_=0, to=9, width=2, font=("Arial", 14), justify='center')
...
tk.Label(
    tab_settings,
    text="Tracker app name (macOS):",
    font=("Arial", 10),
).pack(anchor='w', padx=20, pady=(10, 0))
tracker_app_var = tk.StringVar(value="Upwork")
tk.Entry(tab_settings, textvariable=tracker_app_var, width=30).pack(anchor='w', padx=20, pady=(2, 0))

use_modifiers_var = tk.BooleanVar(value=(platform.system() != "Darwin"))
tk.Checkbutton(
    tab_settings,
    text="Allow modifier shortcuts (Cmd/Ctrl)",
    variable=use_modifiers_var,
).pack(anchor='w', padx=20, pady=(5, 0))
